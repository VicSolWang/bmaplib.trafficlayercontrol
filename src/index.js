...
(function () {
	...
	baidu.dom.removeClass = function (element, className) {
		...
		let lenOld;
		const lenDel = newClasses.length;
		let j;
		let i = 0;
	...
	baidu.dom.getComputedStyle = function (element, key) {
		...
		const doc = baidu.dom.getDocument(element);
		let styles;
	...
	baidu.dom.getDocument = function (element) {
		element = baidu.dom.g(element);
		return Number(element.nodeType) === 9
			? element
...
const stylePrefix = baidu.isMobile() ? '_mobile' : '_deskTop';
...
function create(tag, _attr) {
	...
	_attr = _attr || {};
	// 设置属性
	for (const _name in _attr) {
		const value = _attr[_name];
		const name = result()[_name] || _name;
		if (name === 'style') {
